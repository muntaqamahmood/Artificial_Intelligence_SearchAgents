...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        # startState will be a tuple of pacmanPos and 4cornersPos (Hint1)
    ...
    def isGoalState(self, state: Any):
        ...
        # corners = state[1]  # state[0] is startState, state[1] has all corners
        # # if corner exists, not Goal State
        # if len(corners) == 0:
        #     return False
        # # no corners, Goal!
        # return True
    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [
            ...
            corners = [item for item in state[1]]
            # if next coord is not a wall
            if not hitsWall:
                # if corner
                if (nextx, nexty) in corners:
                    corners.remove((nextx, nexty))
                # legal, add the triple tuple to succ list
                successors.append(
                    (((nextx, nexty), corners), action, 1)
                )  # (Hint2) cost=1
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    "*** YOUR CODE HERE ***"
    startState, cornersList = state[0], list(state[1])
    heur = 0
    while cornersList:
        # calc dist from start state to each of the corners
        dist = [
            (util.manhattanDistance(startState, corner), corner)
            for corner in cornersList
        ]
        # take min dist and the corner that is min dist
        min_dist, nearest_corner = min(dist)
        # set curr pos tonearest corner
        startState = nearest_corner
        # update heurstic with min dist
        heur += min_dist
        # del visited corner
        cornersList.remove(nearest_corner)
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    # if no food left, heur=0
    if not food_grid_List:
        return 0
    # store distances from curr state to all food left coords
    dist = [util.manhattanDistance(position, food) for food in food_grid_List]
    # print(max(dist))
    # take max of distances to be optimistic
    return max(dist)
